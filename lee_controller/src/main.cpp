...
CONTROLLER::CONTROLLER(): _first_odom(false), _new_plan(false) {
    ...
    _acceleration_thrust_pub= _nh.advertise<std_msgs::Float32MultiArray>(_model_name + "/cmd/acc_thr",0);
...
void CONTROLLER::ctrl_loop() {
   ...
   std_msgs::Float32MultiArray acc_thrust;
    ...
    while( ros::ok() ) {
        ...
        lc.controller( mes_p, des_p, mes_q, mes_dp, des_dp, des_ddp, des_yaw, mes_w, &ref_rotor_velocities,&ref_acceleration_thrust);
